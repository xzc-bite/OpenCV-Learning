...
本项目是一个基于**经典计算机视觉**的数字串识别程序，核心采用**形态学图像预处理** + **模板匹配（Template Matching）** 实现。适用于背景相对干净、数字排列整齐的场景（如信用卡号、银行卡号、印刷体数字等）。
...
| 阶段 | 目标 | 核心操作 |
|------|------|----------|
| **模板预处理** | 建立标准数字模板库 `0-9` | 灰度化 → OTSU 二值化 → 轮廓提取 → 尺寸归一化 |
| **输入图像识别** | 定位并识别目标图片中的数字 | 形态学预处理 → 边缘增强 → 轮廓定位 → 模板匹配 |
...
OTSU 原理：由大津展之提出，通过最大化类间方差自动确定最佳阈值 T 。
设图像像素被阈值 T  分为两类 C 
0
​
  （前景）和 C 
...
2.4 轮廓排序与模板字典构建
Python
refcnts, boxs = imutils.contours.sort_contours(refcnts, method="left-to-right")
...
3.1 尺寸归一化
Python
input = imutils.resize(input, width=600)
...
高斯模糊：ksize=(3,3), sigma=0，利用高斯核卷积抑制高频噪声。sigma=0 时 OpenCV 自动根据核大小计算 σ=0.3×((ksize−1)×0.5−1)+0.8 。
...
表格
参数	取值	设计依据
模板尺寸 57×88	固定	经验值，保证数字笔画有足够分辨率，同时控制计算量
...
宽高比 2.5~5.0	经验	针对 16 位信用卡号（4组×4位）的整体外接矩形比例
...
表格
包	版本要求	用途
opencv-python	4.x	核心图像处理库
numpy	1.19+	数组运算与 argmax
imutils	0.5+	轮廓排序与等比 resize 封装
十、快速开始
